(() => {
  ...
  function updateAnimalAmbient(now) {
    ...
    for (const walker of walkers) {
      const npcId = walker.rec?.id;
      if (!npcId) continue;
      if (window.AmbientDialogue?.hasActiveGreetingFor?.(npcId) || window.AmbientDialogue?.hasPendingPlayerGreetingFor?.(npcId)) continue; // Existing or dwell-pending player greeting owns this encounter; the 700 ms animal scan retries later without any new per-frame work.
