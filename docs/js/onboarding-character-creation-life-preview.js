...
`;
  ...
  async function installStarterWeapon(model) {
    ...
    if (scale !== 1 && Number.isFinite(centroidY)) {
      const finalPoint = new THREE.Vector3(toolBase.position.x + toolHolder.position.x, toolBase.position.y + toolHolder.position.y, toolBase.position.z + toolHolder.position.z); // Existing finished local pose.
