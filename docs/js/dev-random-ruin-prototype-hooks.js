...
(() => {
  ...
  function installElevatorSocket(socket) {
    ...
    // Its individual meshes are rasterized by the shared tile snapshot; the
    // moving dais itself remains the independently sampled support surface.
  }

  function installElevatorPushBlock(block) {
    recordCount('elevatorPushBlock');
    recordHandled('elevator-riding push block discovery');
    // Dynamic collision follows this block through the shared occupancy set.
  ...
  function classifyKnownPrototype(object) {
    ...
    if (d.groundedToMovingPlatform) {
      recordCount('groundedToMovingPlatform');
      recordHandled('moving-platform grounded child');
      handled = true;
    }
