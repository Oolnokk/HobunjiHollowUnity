...
(() => {
  ...
  function captureEditorContext() {
    const context = window._bia3dWallContext; // The inline editor exposes its exact r128 renderer instance because r128 defines render() on instances, not WebGLRenderer.prototype.
    if (!context?.renderer || context.renderer.domElement?.id !== 'threeCanvas' || !context?.scene || !context?.camera) return false;
  ...
  function renderedWallHitFromPointer(event) {
    const THREE = window.THREE;
    ...
    raycaster.setFromCamera(ndc, camera);
    const wallRoots = [];
    scene.traverse?.(node => { if (node?.userData?.biaInteriorWallGroup) wallRoots.push(node); });
    const hit = raycaster.intersectObjects(wallRoots, true).find(candidate => candidate?.point && candidate?.face?.normal);
    if (!hit) return null;
    const normal = hit.face.normal.clone().transformDirection(hit.object.matrixWorld);
    normal.y = 0; // Authored brick/cavern triangles can be uneven, but wall furniture remains upright in wall X/Y/normal space.
  ...
  function wallHitFromPointer(event) {
    ...
    const rendered = renderedWallHitFromPointer(event); // What the author sees is authoritative; canonical math remains a fallback for sparse/placeholder wall geometry.
    let best = rendered;
    ...
    lastWallPick = best ? {
      ...
      source: rendered ? 'rendered' : 'canonical',
    ...
    } : { panelId: null, source: null, panelCount: panels.length };
    return best;
  }
  ...
  function boot() {
    ...
    const ready = installUi() && captureEditorContext() && window.WallOrnamentPlacement && window.DaylightWindowRuntime && window._biaBridge;
    if (!ready) {
