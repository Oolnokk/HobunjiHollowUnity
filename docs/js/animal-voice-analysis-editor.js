(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:720px){.voiceAnalysisLegend{display:none}.voiceAnalysisRow{grid-template-columns:1fr 1fr}.voiceAnalysisRow .field{grid-column:1/-1}.voiceAnalysisRow>div:first-child{grid-column:1/-1}}
    `;
  ...
  function install() {
    ...
    let ready = false;
    try {
      ready = typeof renderAnimal === 'function' && typeof VOICE_CLIPS === 'object' && typeof state === 'object' && typeof ensureAnimalOverride === 'function';
    } catch (_) {}
