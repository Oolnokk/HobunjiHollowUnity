(() => {
  ...
  async function _pixelProbeHandler(ev) {
    ...
    if (banubuSnoreDebug) {
      ...
      lines.push(`Banubu snore: status=${banubuSnoreDebug.status || '-'} phase=${banubuSnoreDebug.phase || '-'} night=${banubuSnoreDebug.night ? 1 : 0} playing=${banubuSnoreDebug.playing ? 1 : 0} origin=${banubuSnoreDebug.originSource || '-'}@${banubuSnoreDebug.entranceCol ?? '?'},${banubuSnoreDebug.entranceRow ?? '?'} chunkDist=${banubuSnoreDebug.chunkDistance ?? '-'} horizontal=${Number.isFinite(hDist) ? hDist.toFixed(2) : '-'}t elevation=${Number.isFinite(eDist) ? eDist.toFixed(2) : '-'}t acoustic=${Number.isFinite(aDist) ? aDist.toFixed(2) : '-'}t gain=${Number.isFinite(gain) ? gain.toFixed(2) : '-'} tempo=${Number(banubuSnoreDebug.tempo || 0).toFixed(3)} lastLong=${banubuSnoreDebug.lastLongDurationMs == null ? '-' : Math.round(banubuSnoreDebug.lastLongDurationMs) + 'ms'} lastShort=${banubuSnoreDebug.lastShortDurationMs == null ? '-' : Math.round(banubuSnoreDebug.lastShortDurationMs) + 'ms'} lastPause=${banubuSnoreDebug.lastPauseMs == null ? '-' : Math.round(banubuSnoreDebug.lastPauseMs) + 'ms'}`);
    }
    const banubuClouds = window.BanubuCaveClouds?.debugSnapshot?.(); // On-demand cave height check for a Pixel Probe captured while standing beneath the clouds.
    if (banubuClouds?.length && window.Combat?.deps?.getCurrentArea?.() === 'map_i_den_banubu') {
      lines.push(`Banubu clouds: ${banubuClouds.map(cloud => `${cloud.name} floorY=${cloud.floorY.toFixed(3)} cloudY=${cloud.y.toFixed(3)}`).join(' | ')}`);
    }
    ...
    const meleeAfterimageDebug = window.Combat?.counterShieldAuthoredVisuals?.snapshot?.(); // On-demand shared weapon-ghost state makes ordinary melee afterimages verifiable on mobile.
    const meleeStanceDebug = window.WeaponToolStances?.getRuntimeState?.(); // Used only by this probe to identify the live attack animation feeding the afterimage renderer.
    if (meleeAfterimageDebug) {
      lines.push(`Melee afterimages: eligible=${meleeStanceDebug?.combatMeleeAfterimageEligible ? 1 : 0} active=${meleeStanceDebug?.combatMeleeAfterimage ? 1 : 0} anim=${meleeStanceDebug?.combatAnim || '-'} progress=${meleeStanceDebug?.combatProgress == null ? '-' : Number(meleeStanceDebug.combatProgress).toFixed(2)} phase=${meleeStanceDebug?.combatWindupFrac == null ? '-' : Number(meleeStanceDebug.combatWindupFrac).toFixed(2)}→${meleeStanceDebug?.combatStrikeFrac == null ? '-' : Number(meleeStanceDebug.combatStrikeFrac).toFixed(2)} afflictions=${(meleeStanceDebug?.combatMeleeAfterimageAfflictionIds || []).join(',') || 'none'} colors=${(meleeAfterimageDebug.meleeAfterimageColors || []).join(',') || '#ffffff'} ghosts=${meleeAfterimageDebug.activeMotionTrailGhosts || 0}/${meleeAfterimageDebug.trailMaxGhosts || '-'} renderer=${meleeAfterimageDebug.runtimeActive ? 'active' : 'idle'}`);
    }
    ...
    if (chargedBreakerDebug) {
      ...
      lines.push(`Charged Breaker: active=${chargedBreakerDebug.active ? 1 : 0} held=${Number(chargedBreakerDebug.heldSeconds || 0).toFixed(2)}s pose=${Math.round(Number(chargedBreakerDebug.poseCharge || 0) * 100)}% glowTier=${Number(chargedBreakerDebug.glowOverlayLevel || 0)} flare=${Number(chargedBreakerDebug.glowFlareTier || 0)}:${Math.round(Number(chargedBreakerDebug.glowFlare || 0) * 100)}%${release ? ` last=[pose ${Math.round(Number(release.poseCharge || 0) * 100)}% lunge ${(Number(release.lungePx || 0) / Math.max(1, Number(deps.TILE) || 1)).toFixed(2)}t cone ${Number(release.halfConeDeg || 0).toFixed(1)}° knockback ${Number(release.knockbackPxS || 0).toFixed(0)}px/s gravityResist ${Math.round(Number(release.pitchDistanceResistance || 0) * 100)}% directFlight ${Math.round(Number(release.directFlightStrength || 0) * 100)}%]` : ''}`);
    }
