...
if (/\/tools\/procedural-animation-editor\/(?:index\.html)?$/.test(location.pathname)) {
  ...
  impactTabsScript.addEventListener('load', () => {
    ...
    const swimScript = document.createElement('script'); // Loads the shared runtime/editor swim gait only after the editor's procedural leg adapter seam exists.
    swimScript.id = 'proceduralSwimGaitEditorScript';
    swimScript.async = false;
    swimScript.src = '../../js/procedural-swim-gait.js?v=20260917c';
