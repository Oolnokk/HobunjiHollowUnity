// Routes desktop semantic action keys through the action button actually rendered in that slot.
//
// Touch already proves the button path is authoritative: Plant, Harvest, furniture actions,
// item actions, and tool actions all work when the visible button itself receives pointer input.
// Desktop therefore feeds configured action1..action5 keys into that same pointer path instead
// of recomputing a parallel action list. World Interact (normally E) remains owned by game.js.
(() => {
  'use strict';

  if (window.HobunjiDesktopActionSlotRouter) return;
  ...
  const activePresses = new Map(); // Used to pair keydown/keyup with the exact rendered button pressed, even if the bar refreshes mid-hold.
  const ownedCodes = new Set(); // Used to keep game.js's older array-index fallback from also firing a claimed semantic action key.
  ...
  let lastRouteDebug = null; // Used by diagnostics to report the last desktop action route without browser devtools.
  let badgeSyncPending = false; // Used to coalesce action-bar MutationObserver churn into one key-badge refresh.
  ...
  function actionSlotFromId(actionId) {
    const match = /^action([1-5])$/.exec(String(actionId || '')); // Used to claim only slots that have real visible action buttons.
  ...
  function makePointerEvent(type, pointerId, sourceEvent, button) {
    const rect = button?.getBoundingClientRect?.() || { left: 0, top: 0, width: 0, height: 0 }; // Used to keep synthetic keyboard input stationary at the center so drag-to-aim never starts accidentally.
    const init = {
      bubbles: true,
      cancelable: true,
      pointerId,
  ...
  function finishQHold(event) {
    ...
    const actionId = actionIdForDesktopCode('KeyQ'); // Used so a Q tap follows the configured semantic binding instead of game.js's old "second allowed action" shortcut.
  ...
  function onKeyDown(event) {
    ...
    // Physical Q keeps its existing hold-to-open item-wheel affordance. A quick
    // tap now routes through the actual rendered semantic slot instead of a
    // separately recomputed "second allowed" action.
    if (event.code === 'KeyQ') {
      beginQHold(event);
      claim(event);
      return;
  ...
  function syncRenderedKeyBadges() {
    ...
    for (let index = 0; index < ACTION_BUTTON_IDS.length; index++) {
      const button = document.getElementById(ACTION_BUTTON_IDS[index]); // Used to correct the stale hard-coded E/Q/F3/F4 badges written by game.js after every action-bar refresh.
  ...
  function installBadgeSync() {
    ...
    const stack = document.getElementById('actionStack'); // Used as the narrow observer root because refreshActionBar rewrites only this action UI subtree.
  ...
  // This module loads before game.js. Window-capture ownership makes configured
  // action keys feed the already-working visible button path before the older
  // parallel key-routing code can reinterpret them.
  ...
  window.HobunjiDesktopActionSlotRouter = {
