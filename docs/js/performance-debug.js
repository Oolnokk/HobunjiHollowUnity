...
(function (root) {
  ...
  function profilerText() {
    ...
    const subsystems = [...perfState.subsystem.entries()].sort((a,b) => b[1].avg - a[1].avg).slice(0, 24); // Raised from 5 now that the gameLoop's per-frame work is broken into more (currently up to ~22, depending on which outline settings are on) named buckets — sorted worst-first so the real cost still surfaces even if some future addition pushes the count higher still.
