...
(() => {
  ...
  const FALLBACK_GREHLR_SHOULDER = Object.freeze({"enabled":true,"useSpline":true,"useRun1":false,"splitFrame":true,"splitRightUsesIdle":true,"frameShiftX":0.52,"followFrameShiftX":true,"restGuide":{"a":{"x":0.5423902927484727,"y":0.5694472546137244},"b":{"x":0.9999996666666666,"y":0.572691993389205}},"splinePoints":[{"x":0.5423902927484727,"y":0.5694472546137244},{"x":0.579994260541451,"y":0.6169784772869725},{"x":0.6084907256392884,"y":0.6754227565965308},{"x":0.6278317471892272,"y":0.7447892691279848},{"x":0.6379693486738414,"y":0.8250870055447984},{"x":0.6388555185871044,"y":0.9163247704588463},{"x":0.6304422111109205,"y":1.0185111823033606}]});
  ...
  function canonicalKind(value) {
    ...
    const aliased = normalizeKind(aliases[normalized]);
  ...
  function defaultShoulderRest() {
    ...
    if (normalized) {
      return {
        ...
        frameShiftX: normalized.frameShiftX,
        followFrameShiftX: true,
        restGuide: deepClone(normalized.restGuide),
        splinePoints: deepClone(normalized.splinePoints),
      };
    }
  ...
  function kindFromOptions(options = {}, spriteUrl = '') {
    ...
    for (const species of SPECIES) {
      if (name === species || name.startsWith(species + '-')) return species;
    }
  ...
  const previousHeadRigForKind = typeof renderer?.headRigForKind === 'function'
  ...
  function resolveForKind(kind) {
    ...
    const preview = readPreviewRigs();
    const previewRig = preview[kind] || preview[canonical];
    if (previewRig) return withDefaultShoulder(canonical, previewRig);

    const committed = previousHeadRigForKind?.(canonical)
      || renderer?.ANIMAL_HEAD_RIGS?.[canonical]
      || null;
  ...
  function resolveForOptions(options = {}, spriteUrl = '') {
    const kind = kindFromOptions(options, spriteUrl);
    const direct = options?.headRig || options?.animal?.headRig || null;
  ...
  function install() {
    ...
    renderer.headRigForKind = resolveForKind; // Existing explicit builders now see support rigs and browser previews for the five species.
  ...
  window.HobunjiShoulderSplineProfiles = {
    version: 1,
    species: SPECIES,
    allows: kind => SPECIES_SET.has(canonicalKind(kind)),
    canonicalKind,
    kindFromOptions,
    defaultShoulderRest,
    bodyOnlyRig,
    supportRigFor: resolveForKind,
    resolveForOptions,
    install,
  };
