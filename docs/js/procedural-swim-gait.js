...
//   2) the final composed player body yaw follows the actual resolved movement
//      direction. No swim code in this module translates the player on Y.
//
...
(() => {
  ...
  const BODY_CHANNEL = 'procedural-swim-facing'; // Used by the player-body composer for render-only swim facing.
  const BODY_CHANNEL_PRIORITY = 260; // Used to apply swim facing after ordinary idle/combat yaw deltas while preserving pitch/roll channels.
  ...
  function createLegController(root, options = {}) {
    ...
    function update(dt, requestedStrength, sourceTuning = tuning, refreshNeutral = true) {
      ...
      const targetBlend = strength > 0 ? 1 : 0; // Used to crossfade between upstream ground/idle pose and swim IK.
      state.blend = damp(state.blend, targetBlend, targetBlend > state.blend ? 12 : 16, dt);
  ...
  function runtimeWaterCheck(player) {
    const sharedCheck = window.HobunjiAnimalSubtleElevation?.isInSwimWater; // Preferred existing water detector retained after the old water-Y correction was removed.
  ...
  function syncRuntimeFacingChannel() {
    const composer = window.PlayerBodyTransformComposer; // Used to rotate the entire rendered player rig without touching gameplay position/Y.
    if (!composer?.setChannel || !composer?.clearChannel || !composer?.resolvedYawDeltaRad) return;
    if (!runtimeState.swimming || !runtimeState.hasDirection) {
      composer.clearChannel(BODY_CHANNEL);
      return;
    }
    const root = composer.getPlayerMesh?.(); // Used to read game.js's already-resolved base facing before composer deltas.
    if (!root?.rotation) return;
    composer.clearChannel(BODY_CHANNEL);
    const otherYaw = finite(composer.resolvedYawDeltaRad()); // Used to preserve pre-existing body-yaw channels while solving the exact remaining swim delta.
    const baseYaw = finite(root.rotation.y); // Used as game.js's current resting player facing for this frame.
    const swimYawDelta = wrapSignedAngle(runtimeState.desiredYaw - baseYaw - otherYaw); // Used so base + other channels + swim channel lands exactly on movement direction.
    composer.setChannel(BODY_CHANNEL, {
      priority: BODY_CHANNEL_PRIORITY,
      mode: 'additive',
      rotation: { pitch: 0, yaw: swimYawDelta, roll: 0 },
    });
  }

  function installRuntimeRendererHook(THREE) {
    const proto = THREE?.WebGLRenderer?.prototype; // Used as the final pre-render seam after gameplay facing is resolved.
    if (!proto?.render || proto.__proceduralSwimFacingRenderHook) return;
    const previousRender = proto.render; // Used to preserve every previously-installed render/composer wrapper.
    proto.render = function proceduralSwimFacingRender(...args) {
      syncRuntimeFacingChannel();
      return previousRender.apply(this, args);
    };
    proto.render.__hobunjiProceduralSwimOriginal = previousRender;
    proto.__proceduralSwimFacingRenderHook = true;
  }
  ...
  function installRuntime() {
    ...
    const composer = window.PlayerBodyTransformComposer; // Used to verify the final body composition seam exists before patching.
    if (!legApi?.attach || !THREE || !composer || legApi.__proceduralSwimGaitInstalled) return false;
    ...
    legApi.attach = function proceduralSwimAwareAttach(THREEArg, parent, options = {}) {
      ...
      handle.update = function proceduralSwimPlayerLegUpdate(dt, speedWorldUnitsPerSecond, suppressed, seatedPose) {
        ...
        runtimeState.lastReason = swimming
          ...
          : 'not-swimming';
        if (!swimming) composer.clearChannel(BODY_CHANNEL);
        return result;
      };
      return handle;
    };
    legApi.__proceduralSwimGaitInstalled = true;
    installRuntimeRendererHook(THREE);
...
`;
  ...
  const api = { // Shared public API used by tests, editor tooling, and mobile diagnostics.
    ...
    getDebug() { return { runtime: { ...runtimeState }, editor: { ...editorState }, tuning: { ...tuning }, bodyChannel: BODY_CHANNEL }; },
  };
