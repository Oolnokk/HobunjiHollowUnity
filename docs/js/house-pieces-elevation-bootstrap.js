(() => {
  ...
  // Their farm-side "Enter" object occupies the exterior tile in worldObjects,
  // which generic house placement validation would otherwise mistake for a
  // real blocker. Temporarily remove ONLY those generated door markers while
  // testing/committing a room footprint. If the operation fails, put each
  // untouched marker back; if it succeeds and rebuilds the house, the core
  // throws the old auto feature away and derives/registers a new free wall.
  function _autoEntranceDoorRecords() {
    const records = [];
    if (!deps?.worldObjects) return records;
    for (const entry of deps.getHousePieces?.() || []) {
      for (const feature of entry.features || []) {
        if (feature?.type !== 'entrance' || !feature.autoGenerated || !feature._doorObj || !feature._doorKey) continue;
        if (deps.worldObjects.get(feature._doorKey) !== feature._doorObj) continue;
        records.push({ feature, key: feature._doorKey, doorObj: feature._doorObj });
      }
    }
    return records;
  }
  ...
  function _withAutoEntrancePassThrough(run) {
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (record.feature._doorObj !== record.doorObj) continue;
        if (!deps.worldObjects.has(record.key)) deps.worldObjects.set(record.key, record.doorObj);
      }
    }
  }
