(() => {
  ...
  if (window.NpcFurnitureWardrobes) return;
  ...
  const rootConfig = window.SCRATCHBONES_CONFIG = window.SCRATCHBONES_CONFIG || {}; // Used to expose wardrobe targeting/timing knobs through the normal game config tree.
  rootConfig.game = rootConfig.game || {};
  const authoredConfig = rootConfig.game.npcWardrobes || {}; // Used to preserve project-authored overrides without hard-coding the interaction cadence/radius below.
  const config = rootConfig.game.npcWardrobes = { ...DEFAULT_CONFIG, ...authoredConfig };

  const ACTION_BUTTON_IDS = ['btnAction1', 'btnAction2', 'btnAction3', 'btnItemAction1', 'btnItemAction2']; // Used to inject the contextual wardrobe action into the same five semantic slots as every other world action.
  const mapCache = new Map(); // Used to avoid re-fetching an authored building map every action-bar refresh.
  let activeBinding = null; // Used by pointer/controller dispatch to resolve npc_open_wardrobe to the currently targeted furniture instance.
  let lastArea = null; // Used by mobile diagnostics to show which authored map was inspected most recently.
  let lastTarget = null; // Used by mobile diagnostics to show the exact reticle/furniture match.
  let lastError = null; // Used by mobile diagnostics instead of requiring console access.
  let syncingButtons = false; // Used to prevent our own DOM writes from recursively re-entering the action-stack observer.
  let originalOpenWardrobePanel = null; // Used to keep NpcWardrobe as the single owner of the existing container UI/storage behavior.
    ...
    if (!mapCache.has(area)) {
      const request = fetch(mapUrl(area), { cache: 'no-store' })
        ...
        .catch(error => {
          ...
          return null;
        }); // Used as the one shared in-flight/result promise for this building map.
  ...
  function targetPointTiles(mapData) {
    const debug = window.__hobunjiFurnitureDebug; // Existing runtime bridge used so this module does not duplicate game.js's private player/aim state.
    const player = debug?.playerState; // Used to reproduce the game's ground-reticle projection in tile coordinates.
    const angleDeg = finite(debug?.targetAimAngleDeg, NaN); // Uses the same aim angle desktop mouse/controller/mobile action drag update.
    if (![player?.x, player?.y, angleDeg].every(Number.isFinite)) return null;
    const inputRadius = finite(window.SCRATCHBONES_CONFIG?.game?.input?.targeting?.orbitRadiusTiles, NaN); // Reuses the authored global targeting radius when available.
  ...
  function wardrobeBindings(mapData) {
    const metadataKey = String(config.metadataKey || DEFAULT_CONFIG.metadataKey); // Used to keep the runtime compatible with a future renamed authoring field through config only.
  ...
  function desktopCodeForSlot(slot) {
    const actionId = `action${slot}`; // Used to show the same configurable desktop key badge as the normal action-bar renderer.
  ...
  function actionButton(slotIndex) {
    return document.getElementById(ACTION_BUTTON_IDS[slotIndex] || '');
  }
  ...
  function syncActionButtons() {
    ...
    try {
      ...
      const directButtons = directWardrobeButtons(); // Old character-proximity wardrobe actions game.js may have rendered this frame.
      if (!activeBinding) {
      ...
      for (const button of buttons) {
        if (button !== host && button.dataset.npcFurnitureWardrobeInjected === '1') cleanupInjectedButton(button);
      }
  ...
  function installActionObserver() {
    const stack = document.getElementById('actionStack') || document.body; // Mutation root used to resuppress the legacy NPC action after game.js refreshes the arch.
    if (!stack || stack.dataset.npcFurnitureWardrobeObserved === '1') return false;
    stack.dataset.npcFurnitureWardrobeObserved = '1';
    const observer = new MutationObserver(() => {
      if (syncingButtons) return;
      queueMicrotask(syncActionButtons);
    }); // Used to keep the authored furniture action authoritative without changing game.js's private computeActionButtons().
  ...
  function patchWardrobeEntryPoint() {
    ...
    originalOpenWardrobePanel = wardrobe.openWardrobePanel.bind(wardrobe); // Preserves the existing wardrobe panel/storage UI for furniture-originated opens.
  ...
  installActionObserver();
  tick();
  setInterval(tick, Math.max(50, finite(config.actionSyncMs, DEFAULT_CONFIG.actionSyncMs)));
})();
