...
(() => {
  ...
  const VERSION = 1; // Debug/version gate used by the runtime module loader.
  const HOE_SWING_S = 0.42; // Player chop/hoe visual duration reused by farmer stations.
  const TOOL_W = 0.5; // game.js makeToolPlaneMesh() world width.
  ...
  const CHOP_NEUTRAL = Object.freeze({ x: .03, y: .37, z: -.01, pitch: -155, yaw: -79, bodyYaw: 2, roll: -82 }); // game.js chop neutral.
  const CHOP_WINDUP = Object.freeze({ x: -.18, y: .41, z: -.15, pitch: -165, yaw: 13, bodyYaw: -29, roll: -112 }); // game.js chop windup.
  const CHOP_STRIKE = Object.freeze({ x: 0, y: 0, z: .12, pitch: 13, yaw: -28, bodyYaw: 29, roll: -91 }); // game.js chop strike.
  const BRONZE = Object.freeze({ hex: '#CD7F32', verdigrisHex: '#57B38B' }); // Existing tin-bronze/verdigris palette.
  const WATCHMEN = Object.freeze({
    oddclaw_unumanuk: Object.freeze({ toolKey: 'hatchet', sprite: 'assets/toolsprites/axe_hatchet.png', stance: 'heavy', mastery: 3, xp: 150, verdigris: .5 }), // Mastery 3 = 150/300 = 50% coverage.
    spearhead_unumanuk: Object.freeze({ toolKey: 'fishingspear', sprite: 'assets/toolsprites/harpoon_fishingspear.png', stance: 'light', mastery: 5, xp: 300, verdigris: 1 }), // Mastery 5 = 300/300 = full coverage.
  });
  ...
  const toolKey = value => grips()?.toolKeyFor?.(value) || String(value || '').trim().toLowerCase();
  const loadoutFor = walker => WATCHMEN[walker?.rec?.id] || null;
  const rigFor = walker => walker?.avatarGroup?.userData?.proceduralHandRig || null;
  ...
  function poseQ(pose) {
    ...
    const yaw = (Number(pose?.bodyYaw) || 0) + (Number(pose?.yaw) || 0); // Body yaw is folded into holder-local yaw so walking remains authoritative.
  ...
  function hoePose(progress) {
    const neutral = window.WeaponToolStances?.poses?.hoeTool || CHOP_NEUTRAL; // Same shared hoe neutral used by the player stance runtime.
  ...
  function watchmanPose(loadout) {
    const poses = window.WeaponToolStances?.poses; // Same live Light/Heavy objects used by EnemyWeaponStances.
    if (!poses) return null;
    return loadout.stance === 'heavy' ? poses.heavyWeapon : poses.lightWeapon;
  }

  function scaleFor(key) {
    const scale = Number(grips()?.toolScaleForTool?.(key)); // Shared per-shape tool scale (notably fishing spear).
  ...
  function setHolderPose(walker, holder, pose) {
    ...
    const holderParent = holder?.parent || walker?.root; // Existing station holder parent may differ.
  ...
  function handFrame(walker) {
    ...
    const raw = window.HobunjiHandGripModes?.effectiveFrameForSpecies?.(species) || profileApi.handTransformForSpecies?.(species) || profileApi.modelForSpecies?.(species)?.handFromTool || {};
  ...
  function attachHands(walker, holder, key) {
  ...
  async function buildWatchman(state, loadout) {
    ...
    if (!three || !state.walker?.root) return null;
    ...
    state.buildPromise = (async () => {
      ...
      const plane = new three.Mesh(new three.PlaneGeometry(TOOL_W, TOOL_W * h / Math.max(1, w)), new three.MeshBasicMaterial({ map: texture, transparent: true, alphaTest: .05, side: three.DoubleSide, toneMapped: false })); // Same aspect-preserving flat tool presentation as player meshes.
      plane.rotation.x = -Math.PI / 2;
      plane.renderOrder = RENDER_ORDER;
      plane.layers?.enable?.(1);
      const visual = new three.Group(); // Primary-grip-corrected sprite child.
      visual.add(plane); correctVisual(visual, loadout.toolKey);
      const holder = new three.Group(); // Persistent primary socket driven by shared weapon stance.
      ...
      holder.add(visual);
      (rigFor(state.walker)?.parent || state.walker.root).add(holder);
      state.holder = holder; state.buildPromise = null;
      window.__farmLog?.(`[npc-held-tools] ${state.walker.rec.name || state.walker.rec.id}: ${loadout.toolKey}, ${loadout.stance} stance, mastery ${loadout.mastery}/5, verdigris ${Math.round(loadout.verdigris * 100)}%`, 'combat');
      return holder;
  ...
  function updateState(state) {
    ...
    const loadout = loadoutFor(walker); // Oddclaw/Spearhead persistent watchman loadout, if applicable.
    if (loadout) {
      if (!state.holder) { buildWatchman(state, loadout); return; }
      ...
      attachHands(walker, state.holder, loadout.toolKey);
      state.mode = 'watchman'; state.key = loadout.toolKey;
      return;
    }
    ...
    if (!key || key === 'kurraya' || !walker.stationToolMesh) { state.mode = null; state.key = null; return; }
    ...
    if (key === 'hoe') {
      ...
      if (!loggedHoe.has(walker)) { loggedHoe.add(walker); window.__farmLog?.(`[npc-held-tools] ${walker.rec?.name || walker.rec?.id}: hoe uses player timing/stance/grip + procedural hands`, 'combat'); }
    } else {
      state.mode = 'station-tool'; // Other station tools retain their authored motion but now receive shared grip/hand attachment.
    }
    attachHands(walker, walker.stationToolMesh, key);
    state.key = key;
  }
  ...
  function wrapWalker(walker) {
    ...
    const state = { walker, original, wrapped: null, holder: null, buildPromise: null, buildGeneration: 0, mode: null, key: null, disposed: false }; // Per-walker presentation state.
  ...
  function snapshot(npcId = null) {
    ...
    for (const state of states) {
      ...
      const loadout = loadoutFor(walker); // Supplies watchman mastery/verdigris metadata when applicable.
      rows.push({ id: walker?.rec?.id || null, name: walker?.rec?.name || null, mode: state.mode, toolKey: loadout?.toolKey || state.key, stance: loadout?.stance || (state.key === 'hoe' ? 'hoeTool' : null), mastery: loadout?.mastery ?? null, masteryXp: loadout?.xp ?? null, verdigris: loadout?.verdigris ?? null, holderReady: loadout ? !!state.holder : !!walker?.stationToolMesh, handsReady: !!rigFor(walker), secondaryGrip: !!((loadout?.toolKey || state.key) && grips()?.secondaryGripSpanForTool?.(loadout?.toolKey || state.key)), walkerState: walker?.state || null, area: walker?.area || null });
    }
  ...
  function hookScheduler(api) {
    ...
    const originalInit = api.init.bind(api); // Existing scheduler dependency injection preserved by this wrapper.
    api.init = function npcHeldEquipmentInit(injected) { deps = injected || null; const result = originalInit(injected); installDebug(); scan(); if (scanTimer == null) scanTimer = window.setInterval(scan, 250); return result; };
