...
(function (global) {
  ...
  function buildGroupFromPiece(THREE, piece, bldgMinC, bldgMinR, opts) {
    ...
    for (var i = 0; i < faces.length; i++) {
      ...
      // UVs: ordinary boards/stone retain world-size tiling, but semantic
      // canvas faces are literal cut cloth panels. Each authored canvas face
      // therefore gets one complete canvas.png stretched from 0..1 across
      // its own perimeter rather than continuing the texture into adjacent
      // tent panels or repeating it within a large slope/gable.
      var stretchCanvasFace = tag === 'canvas'; // Used below to select one-full-PNG cloth UVs without changing other building materials.
      ...
      var uLen = stretchCanvasFace ? 1 : v1.distanceTo(v0) / tileSize;
      var vLen = stretchCanvasFace ? 1 : v3.distanceTo(v0) / tileSize;
      var uvs = [
        0, 0,
        uLen, 0,
        uLen, vLen,
