...
(() => {
  ...
  function rebuild(model) {
    ...
    for (const mechanism of model.mechanisms.values()) {
      ...
      const box = worldBox(mechanism.root);
      if (!box || box.max.y - box.min.y <= .35) continue;
  ...
  function create(config) {
    ...
    for (const activator of model.activators) for (const tileKey of objectTiles(activator, model)) model.causes.add(tileKey);
    for (const mechanism of model.mechanisms.values()) for (const tileKey of objectTiles(mechanism.root, model)) model.effects.add(tileKey);
    activeModel = model;
    rebuild(model);
    DS.registerBlocker({
      id:BLOCKER_ID,
