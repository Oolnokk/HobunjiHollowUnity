...
(() => {
  ...
  const prologueRescueMap = new URL('prologue-rescue-map-runtime.js?v=20260907a', base).href; // Loads first so it can hold the real loading screen and inject the authored rescue map before later map/fog modules initialize.
  const prologueRescueZoneCompat = new URL('prologue-rescue-zone-compat.js?v=20260908b', base).href; // Expands the authored 25x25 rescue layout, then session-registers its id as a scripted Cloud Forest EXTERIOR_ZONES entry.
  const prologueSafeArea = new URL('prologue-safe-area-runtime.js?v=20260908a', base).href; // Prevents normal wilderness bandits/wildlife from populating the scripted rescue clearing even though it uses the zone renderer.
  const prologue = new URL('prologue-system.js?v=20260907c', base).href; // Loads the world-owner gameplay-map prologue before onboarding/game.js so it can gate entry and capture normal transition deps.
