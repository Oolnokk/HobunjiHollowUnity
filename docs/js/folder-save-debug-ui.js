...
(() => {
  ...
  const CHANGE_SUMMARY = 'Latest: wilderness treasure meshes are excluded from save JSON, legacy mesh bloat no longer causes false shrink warnings, campfire saves use the guarded manual path, and unreadable character/world files block folder import instead of disappearing.'; // Human-readable current-change summary requested for mobile testing.
  ...
  function diagnosticsSnapshot() {
    ...
    return {
      ...
      recovery: safeSnapshot(() => window.__hobunjiSaveCheckpointDebug?.snapshot?.()),
