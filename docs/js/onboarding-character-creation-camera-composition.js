// Character-creator composition: exact 15° resting turn and a Mao'ao-mid-body camera eye line.
(() => {
  'use strict';
  ...
  const TARGET_PREVIEW_YAW_DEG = 15; // Turns the resting preview fifteen degrees in the user-requested opposite direction from the previous pass.
  ...
  const yawOffsets = new WeakMap(); // Stores only the delta needed to turn each preview root from its existing default to exactly +15°.
  ...
  function yawOffsetFor(root) {
    ...
    if (!yawOffsets.has(root)) {
      ...
      // adds only the fixed delta required to make its first/resting view exactly +15°.
