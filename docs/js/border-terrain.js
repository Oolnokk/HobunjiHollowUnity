(() => {
  ...
  // Procedural border terrain surrounding the playable farm and town grids
  // (the plateau/cliff skirt that fills the world out to the horizon so
  // there's no visible edge) — buildBorderTerrain() for the farm, and its
  // town twin buildTownBorderTerrain() (same plateau-growth pipeline, with
  // canyon gaps cut through wherever a road crosses the border, plus its
  // own sparse grass billboard belt). Extracted out of game.js following
  // the same window.<Namespace> + init(deps) pattern as its sibling
  // systems. Both generators are deterministic (fixed seed) and touch
  // only the scene graph — no player/combat state — so, unlike most of
  // what's left in game.js, this is a clean candidate for eventually
  // running standalone (e.g. server-side world generation for multiplayer).
  let deps = null;
