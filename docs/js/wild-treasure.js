(() => {
  ...
  // Treasure has to be recoverable by digging its own tile. PATH and the
  // wilderness waterway family are all non-diggable, so treat them as one
  // blocked-terrain set for both fresh weekly scatters and save repair.
  function _isTreasureBlockedTileType(type) {
    return type === deps.TileType.PATH
      || type === deps.TileType.RIVER
      || type === deps.TileType.STREAM
      || type === deps.TileType.WATERFALL;
  }

  function _treasureBlockedTerrainTiles(zi) {
    const blockedTiles = []; // Used by new scatters and legacy-placement repair to reserve every non-diggable road/water tile.
    const rowCount = zi.rows ?? zi.grid?.length ?? 0; // Used to scan every row in the active wilderness-zone grid.
    for (let row = 0; row < rowCount; row++) {
      const colCount = zi.cols ?? zi.grid?.[row]?.length ?? 0; // Used to scan every tile in this row for blocked terrain.
      for (let col = 0; col < colCount; col++) {
        if (_isTreasureBlockedTileType(zi.grid?.[row]?.[col]?.type)) blockedTiles.push({ col, row });
      }
    }
    return blockedTiles;
  }
  ...
  // current-week chest under a road or waterway. Preserve that chest's
  // rolled loot, but move it to a deterministic legal tile. If a
  // pathological zone has no legal replacement, drop only that inaccessible
  // placement rather than leaving companion hints pointed at an impossible site.
  function _repairBlockedTreasurePlacements(mapId, persisted) {
    ...
    const blocked = persisted.placements.filter(placement => !placement.found && _isTreasureBlockedTileType(zi.grid?.[placement.row]?.[placement.col]?.type)); // Used as the set of inaccessible legacy road/water placements that need relocation.
    ...
    const blockedTerrainTiles = _treasureBlockedTerrainTiles(zi); // Used to prevent replacement candidates from landing on any road or waterway tile.
    const avoid = [...(deps._zoneReagentPersist.get(mapId)?.placements || []), ...(deps._zoneBerryPersist.get(mapId)?.placements || []), ...blockedTerrainTiles, ...kept]; // Used to keep repairs off plants, berries, blocked terrain, and surviving treasure.
    const rng = deps._mbRng(deps._seedFromString(mapId + ':treasure-access-repair:' + weekIndex())); // Used to make save repair deterministic for this zone/week.
    ...
    deps.debugLog?.(`repairZoneTreasureAccess(${mapId}): relocated ${repaired}, dropped ${blocked.length - repaired} inaccessible road/water chest(s)`);
  ...
  function ensureZone(mapId) {
    ...
    if (persisted?.week === weekIndex()) {
      const repairedBlockedPlacements = _repairBlockedTreasurePlacements(mapId, persisted); // Used to force a rebuild only when this current-week save contained inaccessible road/water treasure.
      if (deps._zoneTreasureMeshGroups.has(mapId) && repairedBlockedPlacements === 0) return; // already built for this week
    } else {
