...
(() => {
  ...
  for (const [key, heightPercentOffset, heightPercentFromFloor, perch, left, right, pixel, portraitModelHeight, placement, portraitScale, handScale, footScale, armLength, derivedHandDefault] of characterRecords) {
    ...
    const legacyPosterior = heightPercentFromFloor == null; // Keeps the one supplied v3 Rakakoan-female posterior readable without inventing v4 data.
    ...
    }; // Preserves the export's authored pixel provenance and coordinate-space markers.
    characters[key] = {
      species,
      gender,
      posteriorRule,
  ...
  window.HOBUNJI_ATTACHMENT_RIG_PROFILE_STATUS = {
    ...
    authoredCharacterProfiles: 12,
    exactSuppliedProfiles: 12,
    authoredCreatureProfiles: 5,
    parrotSharedProfiles: 0,
  ...
  const applyAttachmentRigProfileCorrections = () => {
    ...
    if (handProfiles?.mutate && handScaleUpdates.length) {
      const needsMutation = handScaleUpdates.some(update => Number(handProfiles.data?.speciesScaleOverrides?.[update.species]?.[update.gender]) !== update.handScale);
