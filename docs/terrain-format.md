...
The topmost submap is authoritative across its complete extent: a missing child
entry clears any value authored beneath it on a parent map. At missing cells and
beyond root-map boundaries the sampled value is zero.

Map Editor diffs encode visual-height additions and replacements in
`visualHeights.set`, and erased sparse entries in `visualHeights.removed`.
