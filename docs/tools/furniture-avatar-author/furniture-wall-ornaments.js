...
function refreshPreview() {
  ...
  const origin = new THREE.Mesh(new THREE.SphereGeometry(0.045, 12, 8), new THREE.MeshBasicMaterial({ color: 0xff4f68, depthTest: false })); // Red point marks the actual furniture-local origin, not the wall anchor.
  origin.name = 'FurnitureOrigin';
  origin.renderOrder = 10;
  root.add(origin);
  const isWindow = ['simpleWindow', 'crossbarWindow', 'wideWindow'].includes(state.id); // Only these furniture recipes use the exterior brick opening.
  let apertureDimensions = null;
  if (isWindow) {
    const boxes = (state.parts || []).filter(part => part.kind === 'box').map(part => part.transform || {}); // The runtime derives its silhouette from the authored box bounds.
    if (boxes.length) {
      const minX = Math.min(...boxes.map(box => finite(box.x) - Math.abs(finite(box.sx)) / 2));
      const maxX = Math.max(...boxes.map(box => finite(box.x) + Math.abs(finite(box.sx)) / 2));
      const minY = Math.min(...boxes.map(box => finite(box.y) - Math.abs(finite(box.sy)) / 2));
      const maxY = Math.max(...boxes.map(box => finite(box.y) + Math.abs(finite(box.sy)) / 2));
      const width = (maxX - minX) * 0.925; // Highland wall width at its vertically centered exterior slot: (1 + 0.85) / 2.
      const height = maxY - minY; // Exterior furniture and brick cut retain the authored silhouette height.
      apertureDimensions = { width, height };
      const outline = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.PlaneGeometry(width, height)), new THREE.LineBasicMaterial({ color: 0x36f5d0, depthTest: false })); // Cyan rectangle previews the opening at the wall's midpoint; wall taper and slope are applied in game.
      outline.name = 'ExteriorWindowOpeningAtMidWall';
      outline.position.copy(anchor).addScaledVector(wallNormal, 0.012);
      orientPlaneToNormal(outline, wallNormal);
  ...
  if (readout) {
    ...
    readout.textContent = `Red origin (0, 0, 0) · attachment anchor (${record.anchor.map(value => finite(value).toFixed(3)).join(', ')}) · ${apertureDimensions ? `cyan exterior opening at mid-wall ${apertureDimensions.width.toFixed(3)} × ${apertureDimensions.height.toFixed(3)} · ` : ''}attachment ${record.attachmentSurfaceId} · authored normal ${sourceYaw.toFixed(1)}° · wall ${finite(record.previewWallAngleDeg).toFixed(1)}° · furniture yaw delta ${delta.toFixed(1)}° · default normal offset ${finite(record.defaultNormalOffset).toFixed(3)}`;
  }
}
