...
const FF_V27_URL=new URL('../../references/(HA)FoliageGeneratorV27.html',location.href).toString(); // Uses the same checked-out/deployed branch as this editor.
const FF_V27_BASE=new URL('../../references/',location.href).toString(); // Resolves V27's own relative scripts/assets inside srcdoc.
...
function installFoliageUi(){
  ...
  panel.innerHTML=`<h2>Foliage Furniture</h2><div class="muted">Generate a foliage-authored object as read-only geometry, then author an invisible ordinary furniture box as its seat/collision proxy.</div>
    ...
    <button id="ffDownloadRuntime" class="ok">Download Runtime Furniture JSON</button>
  ...
  fq('ffDownloadRuntime').onclick=downloadRuntimeFurniture;
...
async function refreshFoliageCatalog(prefer=''){
  fstatus('Loading branch-local foliage sources…');
  ff.catalog=Object.entries(FF_DEFAULTS).map(([path,defaults])=>({name:fbase(path),path,sha:defaults.sourceSha||''})).sort((a,b)=>a.name.localeCompare(b.name));
  ...
  fstatus(`${ff.catalog.length} branch-local foliage source${ff.catalog.length===1?'':'s'} ready.`);return ff.catalog;
}
...
async function generatorBridge(){
  ...
  ff.generatorPromise=(async()=>{
    const response=await fetch(FF_V27_URL,{cache:'no-store'}); if(!response.ok)throw new Error(`V27 HTTP ${response.status}`); let source=await response.text();
...
async function loadFoliage(path,{resetProxy=true,metadata=null}={}){
  ...
  try{fstatus(`Loading ${fbase(path)}…`);const response=await fetch(`${flocal(path)}?t=${Date.now()}`,{cache:'no-store'});if(!response.ok)throw new Error(`${fbase(path)} HTTP ${response.status}`);const config=await response.json(),entry=ff.catalog.find(row=>row.path===path),d=fdefaults(path);if(!d)throw new Error('No furniture defaults are registered for this foliage source.');
...
function runtimeFurnitureExport(){if(!ff.sourcePath)throw new Error('Load a foliage source first.');const project=exportData(),anchors=(state.seatAnchors||[]).map(runtimeSeatAnchor).filter(Boolean);return{schema:'hobunji_furniture_authored_runtime.v1',key:fstem(ff.sourcePath),sourceSchema:project.schema,footprint:{w:Math.max(1,Math.round(Number(state.tileBase?.footprintW)||1)),d:Math.max(1,Math.round(Number(state.tileBase?.footprintD)||1))},parts:fcopy(project.parts||[]),seatAnchors:anchors,particleEmitters:fcopy(project.particleEmitters||[]),processingWarps:fcopy(project.processingWarps||[]),processTimelines:fcopy(project.processTimelines||[]),stompAttachPoints:fcopy(project.stompAttachPoints||[]),foliageFurniture:fcopy(project.foliageFurniture)};}
function downloadRuntimeFurniture(){try{const data=runtimeFurnitureExport();download(`${data.key}.json`,new Blob([JSON.stringify(data,null,2)],{type:'application/json'}));fstatus(`Downloaded ${data.key}.json · ${data.seatAnchors.length} seat${data.seatAnchors.length===1?'':'s'}.`);}catch(error){fstatus(`Runtime export failed: ${error.message}`,'error');}}
