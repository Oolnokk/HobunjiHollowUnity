...
function renderSelectedNodeEditor(panel,body,tree,node){
  ...
  if(displayType==='accessShop'){
    ...
    specific=`<div class="editField"><label for="editVisualNext">Next node</label><select id="editVisualNext">${nodeTargetOptions(tree,node.next,node.id)}</select></div><div class="editField"><label for="editVisualDuration">Silent duration (sec)</label><input id="editVisualDuration" type="number" min=".05" max="10" step=".05" value="${Math.max(.05,Number(node.durationSec)||1)}"></div><div class="editField full"><div class="editorNote">Visual nodes hide the dialogue shell, keep the conversation/camera active, then advance automatically. Add camera, movement, body, neck, VFX, or quest-commit cues below.</div></div>`;
  ...
  else if(node.type==='visual')wireVisualNodeEditor(tree,node);
  ...
  window.DialogueCutsceneAuthoring?.renderNodeSection?.(tree,node); // Optional cutscene extension appends camera/presentation controls without replacing the core node inspector.
  syncNodeEditorResize();
}
function changeNodeType(node,type){const tree=currentTree();commitMutation(`Changed node type to ${type}`,()=>{const tags=node.tags||[];Object.keys(node).forEach(k=>{if(!['id','type','tags','pos','cameraId','banubuPresentation'].includes(k))delete node[k]});node.tags=tags;if(type==='accessShop')syncAccessShopNode(node,shopRegistry[0]?.id||'generalStoreWares');else{node.type=type;if(type==='text')Object.assign(node,{text:'',next:null,expression:'neutral',expressionHold:2,revealSpeed:'normal'});if(type==='visual')Object.assign(node,{next:null,durationSec:1});if(type==='choice')node.choices=[{label:'',next:null}];if(type==='sequence')Object.assign(node,{slots:[],next:null,exhaustedNext:null})}syncInboundAccessShopActions(tree,node)},{render:'graph',resetScale:true})}
function wireVisualNodeEditor(tree,node){
  $('editVisualNext').addEventListener('change',e=>commitMutation('Changed visual-node connection',()=>{node.next=e.target.value||null},{render:'graph',resetScale:true}));
  $('editVisualDuration').addEventListener('input',e=>commitMutation('Changed visual duration',()=>{node.durationSec=Math.max(.05,Math.min(10,parseFloat(e.target.value)||1))},{render:'none',coalesceKey:`visual-duration:${node.id}`,log:false}));
  $('editVisualDuration').addEventListener('blur',()=>renderGraph());
}
