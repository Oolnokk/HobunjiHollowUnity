...
const wildTreasure = read('docs/js/wild-treasure.js');
...
const crateSideAuthoredFiles = [ // Used below to restrict crate-side art to the requested storage/crate furniture.
  'chest.json',
  'crateStack.json',
  'shippingBox.json',
];
assert.ok(fs.existsSync(path.join(root, 'docs/assets/textures/crate_side.png')), 'crate_side.png asset exists');
for (const file of crateSideAuthoredFiles) {
