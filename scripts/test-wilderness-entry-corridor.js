...
for (let r = 0; r < 8; r++) {
  for (let c = 4; c <= 14; c++) {
    ...
    if (lateral <= 1) {
      ...
      assert.equal(tile.borderEntryGate, true, 'broad terrain-clearance tag stays invisible but available for later zone-aware backfill');
    }
  }
}
...
const backfilled = Object.entries(cloudRoot.tiles)
  .filter(([, tile]) => tile.generatedObjectType === 'copse');
...
console.log('PASS wilderness entry corridor', {
  ordinary: result,
