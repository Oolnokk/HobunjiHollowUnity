...
const actionArc = fs.readFileSync('docs/js/action-arc-ui.js', 'utf8');
const index = fs.readFileSync('docs/index.html', 'utf8');
const pixelProbe = fs.readFileSync('docs/js/pixel-probe.js', 'utf8');

assert.match(actionArc, /id: 'character-view'[\s\S]{0,320}deps\.setCharacterViewMode\(!deps\.characterViewMode\.enabled\)/, 'the modular utilities wheel exposes the character-view toggle through injected game deps');
...
assert.match(index, /game\.js\?v=[^"'\s>]+/, 'the game script remains cache-versioned');
assert.match(index, /pixel-probe\.js\?v=[^"'\s>]+/, 'the mobile diagnostic remains cache-versioned');
