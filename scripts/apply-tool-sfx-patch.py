...
new_dig = '''      "dig": {
      ...
      "pick": {
        "files": [
          "assets/audio/sfx/sfx_pick.mp3"
        ],
        "gain": 1
      },
      "chop": {
        "files": [
          "assets/audio/sfx/sfx_chop.mp3"
        ],
        "gain": 1
      },
      "breakTree": {
        "files": [
          "assets/audio/sfx/sfx_break_tree.mp3"
        ],
        "gain": 1
      },
      "breakRock": {
        "files": [
          "assets/audio/sfx/sfx_break_rock.mp3"
        ],
        "gain": 1
      },'''
config = replace_once(config, old_dig, new_dig, 'object SFX config')
...
game = replace_once(game, "        { anim: 'refillTurnOut',    dur: 1.0  },", "        { anim: 'refillTurnOut',    dur: 1.0,  sfxKey: 'dig' },", 'refill first strike')
game = replace_once(game, "        { anim: 'refillStrikeBack', dur: 0.5  },", "        { anim: 'refillStrikeBack', dur: 0.5,  sfxKey: 'dig' },", 'refill second strike')
game = replace_once(game, "      const CHOP_TREE_STAGES = [1, -1, 1].map(dirSign => ({ pose: true, dirSign, dur: 0.55 }));", "      const CHOP_TREE_STAGES = [1, -1, 1].map(dirSign => ({ pose: true, dirSign, dur: 0.55, sfxKey: 'chop' }));", 'chop stages')
game = replace_once(game, "      const MINE_ROCK_STAGES = [0, 0, 0].map(() => ({ anim: 'thrust', dur: 0.55 }));", "      const MINE_ROCK_STAGES = [0, 0, 0].map(() => ({ anim: 'thrust', dur: 0.55, sfxKey: 'pick' }));", 'mine stages')
