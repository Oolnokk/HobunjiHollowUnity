...
assert.match(gameSource, /emitter\?\.type === 'fire' && emitter\.enabled !== false/,
  'authored furniture with an enabled fire particle emitter is automatically classified as a Fire Hazard');
...
assert.match(editorSource, /data\.fireHazard\|\|\(data\.particleEmitters\|\|\[\]\)\.some/,
  'loading an older flame-emitter asset automatically enables Fire Hazard');
...
assert.equal(hearth.fireHazard, true, 'Hearth is explicitly authored as a Fire Hazard');
...
// User example 2: 7.6 tiles intended, immediately blocked => 7.6 deficit = 760% profile strength.
{
  ...
  const result = Impact.resolve(e, { kind: 'stone', label: 'Rock' }, TILE, {
    dealHealthDamage(_target, amount) { healthDamage += amount; },
  });
