...
assert.match(gameSource, /const rangedThrowSpins = rangedThrowDef\?\.rangedType === 'thrown' && rangedThrowDef\?\.heldSpin === true[\s\S]*heldSpinBasisDeg[\s\S]*heldSpinRevolutions[\s\S]*spinPlane\.rotation\.z = \(effectiveEndFlip \? Math\.PI : 0\) \+ baseRotZ \+ rangedThrowSpinBasisRad - progress \* Math\.PI \* 2 \* rangedThrowSpinRevolutions/, 'Held throw spin must remain on PNG-local Z while the visible end flip composes as an independent 180-degree in-plane offset.');
