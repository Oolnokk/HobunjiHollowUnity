...
assert.match(gameSource, /combatSwingOrbitRigCentroid[\s\S]*activeCameraMode === SHOULDER_SURF_MODE[\s\S]*currentPlayerPerspectiveDirection\(\)[\s\S]*currentPlayerAimAngle\(\)[\s\S]*currentPlayerAimPitch\(\)[\s\S]*avatarCentroidWorld\?\.\(playerMesh\)[\s\S]*centroidToHandY/, 'thrown playback must orbit the complete hand/pose offset around the live avatar centroid; shoulder-surf shares the head perspective ray while normal desktop preserves its established aim angle/pitch.');
