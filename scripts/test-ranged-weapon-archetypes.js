...
assert.match(gameSource, /const rangedThrowSpins = rangedThrowDef\?\.rangedType === 'thrown' && rangedThrowDef\?\.heldSpin === true[\s\S]*heldSpinBasisDeg[\s\S]*heldSpinRevolutions[\s\S]*spinPlane\.rotation\.z = endFlipZ \+ baseRotZ \+ rangedThrowSpinBasisRad - progress \* Math\.PI \* 2 \* rangedThrowSpinRevolutions/, 'Held throw spin must be driven by per-weapon basis/revolution metadata and compose the Tool-Z end flip on that same axis.');
