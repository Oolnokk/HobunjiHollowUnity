...
assert.match(indexSource, /merged-water-renderer\.js\?v=20260922floodrollback1/,
  'the shipped page cache-busts the rolled-back merged-water renderer');
assert.match(indexSource, /water-system\.js\?v=20260922floodrollback1/,
  'the shipped page cache-busts the rolled-back water system');
...
const floodPlane = buildMapWidePlaneSurfaceData({
  ...
  rows: 50,
  yOffset: 0,
  baseline: { visible: true, surfaceY: 0.4, depth: 0.2, coverage: 0.2 },
});
...
assert.deepEqual(floodPlane.depths, [0.2, 0.2, 0.2, 0.2],
  'the rollback keeps flood shading tied to simulated depth');
assert.deepEqual(floodPlane.coverages, [0.2, 0.2, 0.2, 0.2],
  'the rollback keeps flood opacity coverage tied to simulated depth');
assert.deepEqual(floodPlane.flows, [0, 0, 0, 0, 0, 0, 0, 0],
  'the rollback keeps the non-flowing flood baseline free of synthetic sheen flow');
...
assert.match(waterSystemSource, /opacity: 0\.8/,
  'the emergency rollback restores the pre-739 shared merged-water opacity');
assert.doesNotMatch(waterSystemSource, /FLOOD_VISUAL_DEPTH|FLOOD_VISUAL_COVERAGE|FLOOD_VISUAL_FLOW_MAGNITUDE/,
  'the emergency rollback removes flood-only visual profile constants');
assert.doesNotMatch(waterSystemSource, /renderDepth:|renderCoverage:|renderFlowX:|renderFlowZ:/,
  'the emergency rollback removes render-only baseline overrides');
