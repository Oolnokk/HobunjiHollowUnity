...
assert.match(game, /function playerRangedReticleOrbitFrame\(\)[\s\S]*currentPlayerPerspectiveTarget\(\)\?\.point[\s\S]*yawRad: Math\.atan2\(dx, dz\)[\s\S]*pitchRad: Math\.atan2\(dy, horizontal\)/, 'ranged pose frame must derive yaw and vertical aim directly from the shared finite 3D reticle point, not camera pitch sign conventions');
assert.match(game, /function orbitScaledRangedToolPointTowardReticle\(point, reticleFrame[\s\S]*scaleToolWorldPointAroundPlayerCentroid\(point\)[\s\S]*pitchedY = dy \* cosPitch \+ forward \* sinPitch/, 'ready ranged poses must species\/gender-scale first, then rotate forward distance upward when the reticle is physically above the body pivot');
assert.match(game, /const reticleFrame = rangedTracksAim \? playerRangedReticleOrbitFrame\(\) : null[\s\S]*toolAimPitchRad = rangedTracksAim \? \(reticleFrame\?\.pitchRad \?\? currentPlayerAimPitch\(\)\)[\s\S]*orbitScaledRangedToolPointTowardReticle\(toolHolder\.position, reticleFrame\)/, 'idle ranged stance must use the shared reticle point for both orientation and the species-scaled position orbit');
assert.match(game, /const reticleAligned = combatSwingAlignToReticle && activeTool === 'ranged'[\s\S]*const reticleFrame = reticleAligned \? playerRangedReticleOrbitFrame\(\) : null[\s\S]*orbitScaledRangedToolPointTowardReticle\(toolHolder\.position, reticleFrame\)/, 'ranged fire\/throw animation poses must use the same shared reticle frame after species\/gender scaling');
