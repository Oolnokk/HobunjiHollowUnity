...
assert.equal(windowObject.BanubuSnore.debugSnapshot().lastPauseMs, 6750, 'silence lasts half the combined audible duration');
now = 21250;
scheduled({ timestamp: now });
assert.equal(calls.length, 2, 'the next pair cannot begin before the shortened silence');
now = 21251;
scheduled({ timestamp: now });
