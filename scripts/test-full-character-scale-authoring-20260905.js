...
const portraitScales = {
  'tletingan::male': 0.85,
  'tletingan::female': 0.85,
  'engh-sho::male': 0.95,
  'engh-sho::female': 0.95,
  'mao-ao::male': 1,
  'mao-ao::female': 0.8,
  'kenkari::male': 0.75,
  'kenkari::female': 0.75,
...
for (const [key, tuple] of Object.entries(expected)) {
  ...
  if (portraitScales[key]) assert.ok(Math.abs(tuple.head * portraitScales[key] * 100 - 75) < 1e-9, `${key} raw-PNG Head must equal 75%`);
}
