...
assert(Math.abs(profiles.characters['mao-ao::male'].anchors.leftHandShoulder.position.x - 0.1716052361925984) < 1e-12, 'Mao-ao male left shoulder must retain the recovered 0.9 runtime calibration');
assert(Math.abs(profiles.characters['mao-ao::male'].anchors.leftHandShoulder.position.y - 0.6252801516658442) < 1e-12, 'Mao-ao male shoulder Y must retain the recovered 0.9 runtime calibration');
...
// Reconcile the exact corruption patterns introduced by the v9 bulk export.
const old = JSON.parse(JSON.stringify(profiles));
old.characters['mao-ao::male'].posteriorRule.heightPercentFromFloor = 0;
old.characters['mao-ao::male'].anchors.leftHandShoulder.position = { x: 0.19067248465844266, y: 0.6947557240731601, z: 0 };
...
assert(Math.abs(repaired.characters['mao-ao::male'].anchors.leftHandShoulder.position.x - 0.1716052361925984) < 1e-12, 'uncalibrated v9 shoulder must be repaired');
...
assert(Math.abs(preserved.characters['mao-ao::male'].anchors.leftHandShoulder.position.x - (0.1716052361925984 + 0.0123)) < 1e-12, 'new shoulder authoring must survive master reconciliation');
