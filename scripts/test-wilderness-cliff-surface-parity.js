...
class MeshLambertMaterial {
  constructor({ map = null, hex = '79807c', userData = {} } = {}) {
    this.isMeshBasicMaterial = false;
...
function terrainGeometry() {
  return {
    userData: {
      hobunjiSurfaceStretchSignature: 'stale-pre-final-pass',
...
function plateauMesh() {
  ...
  const mesh = new Mesh(terrainGeometry(), [grass, litRock]);
  mesh.name = 'plateau_under_test';
  return { mesh, grass, litRock };
}
...
function singleCliff(surface, { map = null, hex = '79807c' } = {}) {
  ...
  const mesh = new Mesh(terrainGeometry(), material);
  mesh.userData.naturalSurface = surface;
  mesh.name = `${surface}_under_test`;
  return mesh;
}

const queued = [];
const mapperCalls = [];
const naturalizeCalls = [];
const context = {
  console,
  performance: { now: () => 1234 },
  queueMicrotask: fn => queued.push(fn),
  THREE: { Mesh, Scene, PlaneGeometry },
};
...
context.window.NaturalSurfaceMaterials = {
  naturalizeMesh(mesh, surface, mapping) {
    ...
    mesh.material = new MeshBasicMaterial({
      map: { kind: 'carved_smooth_body_tinted' },
      hex: 'ffffff',
      userData: { naturalSurface: 'rocks' },
    });
