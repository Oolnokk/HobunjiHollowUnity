...
assert.strictEqual(colorPools.cavern.surfaceMaterial, 'farm-cliff', 'Color Pools cavern must use the same farm-cliff surface preset as Banubu\'s working main cave');
...
const builtColorPools = context.CavernGenerator.synthesizeLocaleCavernMapData(colorPools); // Regression guard: the hidden room must carry the same textured farm-cliff shell preset into the shared renderer.
assert.strictEqual(builtColorPools.mesh.surfaceMaterial, 'farm-cliff', 'Color Pools synthesized shell carries Banubu-compatible farm-cliff material metadata');
