...
- `docs/js/wilderness-map-generator.js`'s `stampLocales` places every `great_fey_shrine`-category locale into a zone during a Tothal Shift: it reserves a flat, clear rectangle (footprint + clearance buffer) via the generator's existing occupancy system so nothing else can overlap it, paints the locale's own tiles into the live grid, and registers a `structure` object with a `pathAnchor` so `generatePaths()` routes a path to it like any other landmark — exposed as `workspace.localeInstances`. `docs/game.js`'s `performTothalShift` feeds each zone the locales not yet successfully placed elsewhere this shift, so a singleton locale lands in exactly one of the four zones. Leaf & Pahu's House and the Researcher's Tent are **not** stamped this way — they already had a fixed, non-relocating anchor via `TOTHAL_PRESERVED_TRANSITIONS` (a real building interior is meant to be authored there later), so they're listed as constant `FIXED_LOCALE_LANDMARKS` instead.
- The wilderness map: a per-zone fog-of-war (`_zoneLayouts`-scoped, revealed in a radius around the player, wiped whenever that zone's Tothal year goes stale) drives a top-right minimap widget (visible only inside a wilderness zone) and a full "🗺 Map" tab in the pause menu (press `M`), both in `docs/game.js`/`docs/index.html`/`docs/style.css`. Discovering a locale (walking within the reveal radius) permanently flags it as found in `hobunjiSaveMeta` — the map always draws discovered locales from their *live* current placement rather than a remembered position, so a shrine's marker still tracks it correctly after a later Tothal Shift moves it.
